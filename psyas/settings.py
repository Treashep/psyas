...
# 3. 安全配置
SECRET_KEY = env.str("SECRET_KEY", default="dev-secret-key")  # 开发环境默认密钥（生产环境必须通过 .env 设置）
BCRYPT_LOG_ROUNDS = env.int("BCRYPT_LOG_ROUNDS", default=13)  # 密码加密强度


# 4. 静态文件配置
SEND_FILE_MAX_AGE_DEFAULT = env.int("SEND_FILE_MAX_AGE_DEFAULT", default=300)  # 静态文件缓存时间（秒）
...
CACHE_REDIS_URL = env.str("CACHE_REDIS_URL", default="")  # 若用 Redis，通过环境变量配置地址
